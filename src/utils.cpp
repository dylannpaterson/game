...
#include <random> // <<< Include for modern C++ random numbers
...
SDL_Texture *renderText(SDL_Renderer *renderer, TTF_Font *font,
  ...
  if (!font) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "renderText Error: Font is null.");
    return nullptr;
  }
  if (!renderer) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "renderText Error: Renderer is null.");
    return nullptr;
  }
  ...
  SDL_FreeSurface(
      textSurface); // Free surface regardless of texture creation success
  return textTexture;
}
...
SDL_Context initializeSDL(int width, int height) {
  ...
  if (!(IMG_Init(imgFlags) & imgFlags)) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                 "SDL_image could not initialize! IMG_Error: %s\n",
                 IMG_GetError());
    TTF_Quit();
    SDL_Quit();
    return context; // Return empty context on error
  ...
  context.window = SDL_CreateWindow(
      "Wizard Roguelike", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
      width, height,
      SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE); // Added Resizable flag
...
void cleanupSDL(SDL_Context &context) {
  if (context.renderer != nullptr) {
    ...
    context.renderer =
        nullptr; // Good practice to null pointers after destroying
  }
  if (context.window != nullptr) {
    SDL_DestroyWindow(context.window);
...
bool findNearestValidTarget(const GameData &gameData, int spellIndex,
  ...
  if (spell.targetType != SpellTargetType::Enemy) {
    SDL_Log("DEBUG: [NearestTarget] Spell '%s' cannot auto-target (not Enemy "
            "target type).",
            spell.name.c_str());
    return false;
  }
  ...
  int closestDistSq = std::numeric_limits<int>::max(); // Use max integer value
                                                       // as initial distance
  ...
  for (const auto &enemy : gameData.enemies) {
    ...
    if (isWithinBounds(enemy.x, enemy.y, gameData.currentLevel.width,
      ...
      visibility =
          gameData.visibilityMap[enemy.y]
                                [enemy.x]; // Get visibility value from the map
    }
    if (visibility <= 0.0f)
      continue; // Skip unseen enemies
    ...
    if (distSq <= effectiveRange * effectiveRange) {
      ...
      // if (hasLineOfSight(playerTileX, playerTileY, enemy.x, enemy.y,
  ...
  if (nearestEnemy) {
    ...
    SDL_Log("DEBUG: [NearestTarget] Found nearest enemy %d at [%d, %d]",
            nearestEnemy->id, outTargetPos.x, outTargetPos.y);
    return true; // Target found
  } else {
    SDL_Log("DEBUG: [NearestTarget] No valid enemy found in range/sight for "
            "spell '%s'.",
            spell.name.c_str());
    return false; // No target found
  }
  // --- FIXED: Added explicit return for the case where no target is found ---
  // return false; // This line is now technically redundant due to the else
  // block above, but ensures clarity.
}

int rollDice(int numDice, int dieType, int bonus) {
  if (numDice <= 0 || dieType <= 0) {
    return bonus; // Only return bonus if no dice are rolled
  }

  // --- Use C++ <random> for better randomness ---
  static std::random_device rd; // Will be seeded once per application run
  static std::mt19937 gen(
      rd()); // Standard mersenne_twister_engine seeded with rd()
  std::uniform_int_distribution<> distrib(1, dieType);
  ...
  // --- Alternative using <cstdlib> rand() (less preferred) ---
  // #include <cstdlib> // Requires srand() called once in main()
  // int total = 0;
  // for (int i = 0; i < numDice; ++i) {
  //     total += (rand() % dieType) + 1; // Basic roll
  // }
  // return total + bonus;
  // --- End rand() ---
}
