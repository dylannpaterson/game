      ...
      if (currentAppState == AppState::Gameplay &&
        ...
        if (event.button.button ==
          ...
          if (gameData.currentSpellIndex != -1) {
            ...
            if (distance <= spell.range) { // Check if target is within range
...
void updateLogic(GameData &gameData, AssetManager &assets, float deltaTime) {
  ...
  void renderScene(GameData & gameData, AssetManager & assets) {
      ...
      if (visibility > 0.0f) { // Only render if the tile is visible
        ...
        if (itemTexture) {
          ...
          SDL_Rect fallbackRect = {
              (item.x * gameData.tileWidth) - gameData.cameraX +
                  gameData.tileWidth / 4,
              (item.y * gameData.tileHeight) - gameData.cameraY +
                  gameData.tileHeight / 4,
              gameData.tileWidth / 2, gameData.tileHeight / 2};
    ...
    if (gameData.currentPedestal.has_value()) {
      const RunePedestal& pedestal = gameData.currentPedestal.value(); // Use const ref
      ...
      if (visibility > 0.0f) { // Only render if visible tile
          ...
          if (pedestal.isDeactivating) {
              ...
              if (!pedestal.deactivationFrameTextureNames.empty() &&
                  ...
                  pedestal.currentFrame < pedestal.deactivationFrameTextureNames.size())
              {
                  textureKey = pedestal.deactivationFrameTextureNames[pedestal.currentFrame];
              } else if (!pedestal.deactivationFrameTextureNames.empty()) {
                   // Fallback to last frame if index is somehow invalid during deactivation
                   textureKey = pedestal.deactivationFrameTextureNames.back();
                   SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Invalid frame index %d during pedestal deactivation, using last frame.", pedestal.currentFrame);
              }
          } else if (pedestal.isActive) {
                  ...
                  pedestal.currentFrame < pedestal.idleFrameTextureNames.size())
              {
                   ...
                   textureKey = pedestal.idleFrameTextureNames.front();
                    SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Invalid frame index %d during pedestal idle, using first frame.", pedestal.currentFrame);
              }
          } else {
              // --- Render Deactivated State (Last frame of deactivation) ---
              if (!pedestal.deactivationFrameTextureNames.empty()) {
                  textureKey = pedestal.deactivationFrameTextureNames.back(); // Use the last frame
              } else {
                   SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Cannot render deactivated pedestal: No deactivation frames defined.");
                   // Optionally render a fallback placeholder here if textureKey remains empty
              }
          }
          ...
          if (pedestalTexture) {
              SDL_Rect pedestalRect = { /* ... calculate rect ... */
              ...
              // Optional: Render placeholder if texture failed to load or key was empty
               if (!textureKey.empty()) {
                   SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Pedestal texture '%s' not found!", textureKey.c_str());
               }
               // Draw simple placeholder (e.g., gray square)
               SDL_Rect fallbackRect = {(pedestal.x * gameData.tileWidth) - gameData.cameraX, (pedestal.y * gameData.tileHeight) - gameData.cameraY, gameData.tileWidth, gameData.tileHeight};
               Uint8 alpha = static_cast<Uint8>(visibility * 128);
               SDL_SetRenderDrawBlendMode(gameData.renderer, SDL_BLENDMODE_BLEND);
               SDL_SetRenderDrawColor(gameData.renderer, 100, 100, 100, alpha); // Gray fallback
               SDL_RenderFillRect(gameData.renderer, &fallbackRect);
               SDL_SetRenderDrawBlendMode(gameData.renderer, SDL_BLENDMODE_NONE);
          }
      }
    ...
    if (isTargeting) {
      ...
      if (!gameData.currentGamePlayer.targetingFrameTextureNames.empty() &&
          gameData.currentGamePlayer.currentTargetingFrame <
              gameData.currentGamePlayer.targetingFrameTextureNames.size()) {
        textureKeyToUse =
            gameData.currentGamePlayer.targetingFrameTextureNames
                [gameData.currentGamePlayer.currentTargetingFrame];
