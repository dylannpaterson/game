...
Level generateLevel(int width, int height, int maxRooms, int minRoomSize,
                    int maxRoomSize, std::vector<Enemy> &enemies, int tileW,
                    int tileH, std::optional<SDL_Point> &outPedestalPos) {
  Level level;
  level.width = width;
  level.height = height;
  level.tiles.resize(height,
                     std::string(width, 'V')); // 1. Make everything void
  ...
  for (int i = 0; i < maxRooms; ++i) {
    ...
    SDL_Rect newRoom = {roomX, roomY, roomWidth + 2,
                        roomHeight + 2}; // Increase size for walls
    bool overlaps = false;
    ...
    if (!overlaps) {
      ...
      //     if (isWithinBounds(x, roomY, width, height)) level.tiles[roomY][x]
      //     = '#'; // Top wall if (isWithinBounds(x, roomY + roomHeight + 1,
      //     width, height)) level.tiles[roomY + roomHeight + 1][x] = '#'; //
      //     Bottom wall
      // }
      // for (int y = roomY; y < roomY + roomHeight + 2; ++y) {
      //     if (isWithinBounds(roomX, y, width, height)) level.tiles[y][roomX]
      //     = '#'; // Left wall if (isWithinBounds(roomX + roomWidth + 1, y,
      //     width, height)) level.tiles[y][roomX + roomWidth + 1] = '#'; //
      //     Right wall
      // }
    }
  }
    ...
    for (int i = 1; i < numRooms; ++i) {
      if (parent[i] != -1) { // Ensure parent exists
        ...
        if (!isWithinBounds(x1, y1, width, height) ||
            !isWithinBounds(x2, y2, width, height))
          continue;
  ...
  if (!rooms.empty()) {
    ...
    do {
      level.startCol =
          rooms[startRoomIndex].x + 1 +
          (rand() %
           std::max(1,
                    rooms[startRoomIndex].w -
                        2)); // Use max(1,..) to avoid modulo by zero/negative
      level.startRow = rooms[startRoomIndex].y + 1 +
                       (rand() % std::max(1, rooms[startRoomIndex].h - 2));
      startAttempts++;
    } while ((!isWithinBounds(level.startCol, level.startRow, width, height) ||
    ...
    if (rooms.size() > 1) {
      ...
      do {
        endRoomIndex = roomDist(gen);
               ...
               startRoomIndex); // Ensure start and end are in different rooms
      // Place end in a random valid floor tile within the end room
      int endAttempts = 0;
      do {
        level.endCol = rooms[endRoomIndex].x + 1 +
                       (rand() % std::max(1, rooms[endRoomIndex].w - 2));
        level.endRow = rooms[endRoomIndex].y + 1 +
                       (rand() % std::max(1, rooms[endRoomIndex].h - 2));
        endAttempts++;
      } while ((!isWithinBounds(level.endCol, level.endRow, width, height) ||
      ...
      int endAttempts = 0;
      do {
        level.endCol = rooms[startRoomIndex].x + 1 +
                       (rand() % std::max(1, rooms[startRoomIndex].w - 2));
        level.endRow = rooms[startRoomIndex].y + 1 +
                       (rand() % std::max(1, rooms[startRoomIndex].h - 2));
        endAttempts++;
      } while (
          ((level.endCol == level.startCol && level.endRow == level.startRow) ||
           !isWithinBounds(level.endCol, level.endRow, width, height) ||
           level.tiles[level.endRow][level.endCol] != '.') &&
          endAttempts < 100);
    }
    // Handle cases where placement failed after attempts
    if (!isWithinBounds(level.startCol, level.startRow, width, height) ||
        level.tiles[level.startRow][level.startCol] != '.') {
      SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                  "Failed to place start point in a valid floor tile!");
    ...
    if (!isWithinBounds(level.endCol, level.endRow, width, height) ||
        level.tiles[level.endRow][level.endCol] != '.') {
      SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                  "Failed to place end point in a valid floor tile!");
      level.endCol = level.startCol + 1;
      level.endRow = level.startRow;
      if (isWithinBounds(level.endCol, level.endRow, width, height))
        level.tiles[level.endRow][level.endCol] = '.';
    }
  ...
  if (!rooms.empty()) {
    ...
    do {
      ...
      if (pedestalAttempts == 1) {
        pedestalPos.x =
            rooms[pedestalRoomIndex].x + rooms[pedestalRoomIndex].w / 2;
        pedestalPos.y =
            rooms[pedestalRoomIndex].y + rooms[pedestalRoomIndex].h / 2;
      } else {
        // Random position within the room's floor area
        pedestalPos.x = rooms[pedestalRoomIndex].x + 1 +
                        (rand() % std::max(1, rooms[pedestalRoomIndex].w - 2));
        pedestalPos.y = rooms[pedestalRoomIndex].y + 1 +
                        (rand() % std::max(1, rooms[pedestalRoomIndex].h - 2));
      }
      ...
      if (isWithinBounds(pedestalPos.x, pedestalPos.y, width, height) &&
          ...
          !(pedestalPos.y == level.startRow &&
            pedestalPos.x == level.startCol) &&
          !(pedestalPos.y == level.endRow && pedestalPos.x == level.endCol)) {
        // Valid spot found!
        outPedestalPos = pedestalPos; // Assign to the output parameter
        SDL_Log("INFO: Placed Rune Pedestal at [%d, %d].", pedestalPos.x,
                pedestalPos.y);
        // Optional: Mark the tile differently? Or handle via
        // GameData.currentPedestal presence.
    ...
    if (!outPedestalPos.has_value()) {
      SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                  "Failed to place Rune Pedestal after %d attempts!",
                  MAX_PEDESTAL_ATTEMPTS);
      // Handle failure case - maybe try another room or skip placement for this
      // level?
    }
  } else {
    SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                "Cannot place pedestal - level has no rooms!");
  }
  // --- *** END Place Rune Pedestal *** ---

  // 5. Spawn enemies
  int numEnemiesToSpawn =
      3 + level.rooms.size() / 2; // Example: Scale with number of rooms
  numEnemiesToSpawn = std::min(numEnemiesToSpawn,
                               12); // Cap at max enemy count (adjust as needed)

  int spawnedCount = 0;
  int spawnAttempts = 0;
  int maxSpawnAttemptsTotal = width * height; // Safety limit

  while (spawnedCount < numEnemiesToSpawn &&
         spawnAttempts < maxSpawnAttemptsTotal) {
    spawnAttempts++;
    int spawnX = rand() % width;
    int spawnY = rand() % height;

    // Check if tile is floor and not start/end
    if (isWithinBounds(spawnX, spawnY, width,
                       height) && // Check bounds before accessing tiles
      ...
      if (!occupied) {
        ...
        int newId =
            Enemy::getNextId(); // Get next ID using the public static method
        enemies.emplace_back(newId, EnemyType::SLIME, spawnX, spawnY, tileW,
                             tileH);
        // *****************************************************
        spawnedCount++;
      }
  ...
  if (spawnedCount < numEnemiesToSpawn) {
    SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                "Could only spawn %d out of %d requested enemies.",
                spawnedCount, numEnemiesToSpawn);
  }
