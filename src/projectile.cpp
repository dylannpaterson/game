...
Projectile::Projectile(ProjectileType pType, SDL_Texture* tex, int w, int h,
    ...
    : type(pType),
      ...
      startX(sX), startY(sY),
      targetX(tX), targetY(tY), // Store initial target coords
...
void Projectile::calculateDirection(float toX, float toY) {
    ...
    if (magnitude > 0.0001f) { // Avoid division by zero
        ...
        // isActive = false;
        // SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION, "Projectile direction calculation resulted in zero magnitude.");
    }
}
...
bool Projectile::update(float deltaTime, const GameData& gameData) {
    ...
    if (targetEnemyId != -1) {
        ...
        if (targetEnemy != nullptr && targetEnemy->health > 0) {
            ...
            SDL_Log("Projectile target Enemy ID %d not found or dead. Deactivating.", targetEnemyId);
            isActive = false;
    ...
    float hitThresholdSq = 10.0f * 10.0f; // Example: Hit if within 10 pixels squared

    // Check if we will reach or pass the target this frame OR if we are already very close
    if ((distanceMoved * distanceMoved >= remainingDistSq) || (remainingDistSq < hitThresholdSq)) {
