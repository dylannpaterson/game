// ui.h
#ifndef UI_H
#define UI_H

#include "asset_manager.h"
#include "character.h" // Needed for PlayerCharacter type in function signatures
#include "game_data.h"
#include <SDL.h>
#include <SDL_ttf.h>

struct GameData;
class AssetManager;

// Assuming renderText declaration is in utils.h, which ui.cpp includes, or
// include utils.h here too
#include "utils.h" // Make sure this is included if ui.cpp needs renderText directly

// Existing renderUI declaration

// --- Add declaration for renderCharacterSheet ---
void renderCharacterSheet(GameData &gameData, AssetManager &assets);
// ----------------------------------------------
void renderUI(GameData &gameData, AssetManager &assets);

void renderSpellUnlockMenu(GameData &gameData, AssetManager &assets);

// --- Helper function to convert RuneType to string for display ---
// Declaration here, definition in ui.cpp or utils.cpp
std::string RuneTypeToString(RuneType runeType);

void renderSpellBar(GameData &gameData, AssetManager &assets);

#endif // UI_H