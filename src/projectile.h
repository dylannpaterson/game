...
public:
  ...
  ProjectileType type;
  bool isActive;
  int sourceSpellIndex =
      -1; // <<< ADDED: Index of the spell in caster's knownSpells list

  // --- NEW: Target Tracking ---
  int targetEnemyId =
      -1; // ID of the enemy being targeted (-1 if none/tile target)

  // --- Position & Movement ---
  float startX, startY; // Visual world coordinates where it originated
  float targetX,
      targetY; // Initial visual world coordinates of the target center (still
               // useful for non-homing or if target dies)
  ...
  // --- Constructor (Updated) ---
  Projectile(ProjectileType type, SDL_Texture *tex, int w, int h, float startX,
             float startY, float targetX, float targetY, float speed,
             int damage,
             int spellIdx,       // <<< ADDED: Source spell index
             int targetId = -1); // Added optional targetId parameter

  // --- Methods (Updated) ---
  // Updates position, returns true if target reached/hit this frame, false
  // otherwise Now requires GameData to find the target enemy by ID
